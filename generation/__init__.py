...
def generate_with_retrieval(retrievals, user_prompt, *args, **kwargs):
    ...
    You are an AI assistant created by Latent Health to summarize the current drug information on the market. When doing so:
        ...
        {content if len(content) > 0 else "There is no relevant content useful for the current generation. Please use information you already have to generate an answer that follows the above criteria"}
    ...
    completion = oai.chat.completions.create(
        model="gpt-4-turbo",
        messages=[
            {"role": "system", "content": "INJECT SYSTEM PROMPT HERE"},
            {"role": "user", full_prompt}
        ],
        *args,
        **kwargs,
    )
